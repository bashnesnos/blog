...
Идея такая: локализованные страницы во время сборки отправлять в отдельную папку. Так как блог предполагается статический, то это самый простой и убийственый вариант. Переключение между локализациями будет заключатся в вырезании из/добавления в URL нужного нам куска. Например:

	src\jbake\content\blog\2014\jbake_locale.md -> \blog\2014\jbake_locale.html (оригинальная страница)
	src\jbake\content\blog\2014\en_jbake_locale.md -> \en\blog\2014\jbake_locale.html (локализованная)

Т.е. чтобы переключиться на английскую версию в данном примере, надо будет на оригинальной страниjaце в URL добавить **en\\**. Что и прозойдёт, если клинкуть <a href="/en/blog/2014/jbake_locale.html">сюда</a>
